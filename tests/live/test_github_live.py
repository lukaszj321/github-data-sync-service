...
@pytest.mark.skipif(os.environ.get("RUN_LIVE_TESTS") != "1", reason="RUN_LIVE_TESTS=1 required")
def test_live_fastapi_issues_page_with_since() -> None:
    settings = Settings()
    client = GitHubClient(
        base_url=settings.github_api_base_url,
        user_agent=settings.github_user_agent,
    ...
    try:
        ...
        client.close()
    assert isinstance(page, GitHubIssuePage)
    assert page.fetched_count >= len(page.issues)
    assert page.rate_limit.limit is None or page.rate_limit.limit >= 0
    assert page.rate_limit.remaining is None or page.rate_limit.remaining >= 0
